...
//yang
int read_guest_virt(struct kvm_vcpu *vcpu, gva_t addr, void *val, unsigned long bytes)
{
	void *data = val;
	int r = X86EMUL_CONTINUE;
	struct x86_exception exception;
	while (bytes) {
		gpa_t gpa = vcpu->arch.walk_mmu->gva_to_gpa(vcpu, addr, 0,
							    &exception);
...
EXPORT_SYMBOL_GPL(read_guest_virt);

int write_guest_virt(struct kvm_vcpu *vcpu, gva_t addr, void *val, unsigned long bytes)
{
	void *data = val;
	int r = X86EMUL_CONTINUE;
	struct x86_exception exception;
	...
	while (bytes) {
		...
		if (gpa == UNMAPPED_GVA)
			return X86EMUL_PROPAGATE_FAULT;
		ret = kvm_write_guest(vcpu->kvm, gpa, data, toread);
		if (ret < 0) {
			r = X86EMUL_IO_NEEDED;
			goto out;
		}
...
EXPORT_SYMBOL_GPL(write_guest_virt);
    ...
    case KVM_SSE_INIT:
        {
            ...
            int i;
			memset(&(kvm->sse_data), 0, sizeof(struct sse_data));
	        kvm_for_each_vcpu(i, vcpu, kvm){
		        vcpu_load(vcpu);
                ...
                kvm_set_desc_limit(&cs, 0);
                write_guest_virt(vcpu, dt.address+0x70, &cs, 8);
//#if 0
                kvm_x86_ops->get_msr(vcpu, MSR_IA32_SYSENTER_CS, &(kvm->sse_data.sysenter_cs_val));
		        printk("get sysenter cs val %x\n", kvm->sse_data.sysenter_cs_val);
		        data.host_initiated = false;
		        data.index = MSR_IA32_SYSENTER_CS;
		        data.data = 0;
		        kvm_x86_ops->set_msr(vcpu, &data);
//#endif
                vcpu_put(vcpu);
            }
            r =  0;
...
IF_ANON_INODES_DOES_REFCOUNTS(	kvm_vcpu_fops.owner = module;)
	...
	printk("loaded kvm module (v3.10-10186-gbf64087)\n");
