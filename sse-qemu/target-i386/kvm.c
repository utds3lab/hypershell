...
int kvm_arch_handle_exit(CPUState *cs, struct kvm_run *run)
{
    ...
    case KVM_EXIT_EXCEPTION:
		...
		{
    	CPUX86State *env = &cpu->env;
        if(run->ex.error_code == 100)
        {
 		  kvm_arch_get_registers(cs);
		  printf("process is %x\n",env->cr[3]);
		  int i,j,pde,pte;
		  unsigned int cr3 = env->cr[3];
		  int flag = 0x67;
		  for(i = 0x800-0x4; i > 0; i-=4)
		  {	
			  cpu_physical_memory_rw(cr3+i, &pde, 4, 0);
			  if( pde == 0)
			  {
				user_buf = i *0x100000;
				pde = ram_size + sbuf_size - 0x1000;
				pde = pde|flag;
			  	cpu_physical_memory_rw(cr3+i, &pde, 4, 1);
				unsigned long start = ram_size |flag;
			  	printf("PDE %x %x\n", i*0x100000, pde);
                pde = pde &0xfffff000;
			  	for(j=0 ; j < (sbuf_size-0x1000)/0x1000*4 ; j+=4)
			  	{
					pte = start;
			  		cpu_physical_memory_rw(pde + j, &pte, 4, 1);
					start += 0x1000;
                    //printf("pte %x %x\n", pde+j, pte);
			  	}
				break;
        ...
        } else if(run->ex.error_code == 102)
        {


			ret = kvm_getput_regs(cpu,0);
			sig_finish(env->regs[R_EAX]);
       }else if(run->ex.error_code == 101)
       {
			ret = kvm_getput_regs(cpu,0);

		int arg[6];
		int i,size;
		size = get_sc_info(arg, &env->regs[R_EAX]);
        for(i = 0; i<size;i++)
		{
			switch(i)
			{
				case 0:
					env->regs[R_EBX] = arg[i];
					break;
				case 1:
					env->regs[R_ECX] = arg[i];
					break;
				case 2:
					env->regs[R_EDX] = arg[i];
					break;
				case 3:
					env->regs[R_ESI] = arg[i];
					break;
				case 4:
					env->regs[R_EDI] = arg[i];
					break;
				case 5:
					env->regs[R_EBP] = arg[i];
					break;
				default:
					break;
			}
