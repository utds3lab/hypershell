...
//yang.beign
static pthread_t stero;
void * stero_fn(void *arg)
{
    while(1){
        wait_sc();
        int ret;
        ret = sse_kvm_ioctl(KVM_SSE_START);
        if(ret < 0)
            fprintf(stderr, "can't start the sse\n");
    }
}

void hmp_sse_start(Monitor *mon, const QDict *qdict)
{
    int err;
    sig_init();
    err = pthread_create(&stero, NULL, stero_fn, NULL);
    ...
    int ret;
    ret = sse_kvm_ioctl(KVM_SSE_INIT);
    if(ret < 0)
        fprintf(stderr, "can't init the sse\n");
}
